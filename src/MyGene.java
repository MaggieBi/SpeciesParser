...
public class MyGene extends Gene implements Comparable<MyGene> {
    ...
    public int getDistance(Gene g) {
        return Math.min(distanceRecursive(getDNA(), g.getDNA()), distanceRecursive(g.getDNA(),getDNA()));
    }
    ...
    /** Return the distance between genes s0 and s1, which is the smaller one of the two distances
     *  obtained from comparing s0 to s1 and s1 to s1, as defined in the A3 handout. */
    public static int distanceRecursive(String s0, String s1) {
        ...
        return basicDistance(m[1], m[3]) + Math.min(distanceRecursive(m[4], m[2]),distanceRecursive(m[2], m[4]));   
    }
}
